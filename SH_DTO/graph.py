...
class MyGraph(ScatterLayout):
    xlabel = StringProperty("X-axis")
    ylabel = StringProperty("Y-axis")
    x_ticks_minor = NumericProperty(10)
    x_ticks_major = NumericProperty(100)
    y_ticks_major = NumericProperty(20)
    ...
    def __init__(self, **kwargs):
        ...
        graph_theme = {
            'label_options': {
                    'color': rgb('00ffb3'),  # color of tick labels and titles
                    'bold': True},
                'background_color': rgb('000000'),  # f8f8f2 back ground color of canvas
                'tick_color': rgb('0070ff78'),  # ticks and grid
                'border_color': rgb('0070ff78')}  # border drawn around each graph
        # Create a Graph instance and add it to the Scatter
        self.graph = Graph(
            #xlabel=self.xlabel,
            #ylabel=self.ylabel,
            ...
            y_grid_label=self.y_grid_label,
            x_grid_label=self.x_grid_label,
            padding=self.padding,
            xlog=self.xlog,
            ylog=self.ylog,
            x_grid=self.x_grid,
        ...
        self.plot = SmoothLinePlot(color=rgb('00Af00'))
        ...
        self.widget.add_widget(self.graph)
        
        self.legend_panel = BoxLayout(
        ...
        with self.legend_panel.canvas.before:
            Color(0, 0, 0, 0.7)
            self.legend_bg = Rectangle(pos=self.legend_panel.pos, size=self.legend_panel.size)
        
        self.legend_panel.bind(pos=self._update_legend_bg, size=self._update_legend_bg)

        self.widget.add_widget(self.legend_panel)

        
        self.add_widget(self.widget)
        
        #self.add_widget(self.graph)
        # Легенда знизу графіка
        
        #self.add_widget(self.legend_panel)



    def _update_legend_bg(self, *args):
        """Оновлює фон легенди"""
        self.legend_bg.pos = self.legend_panel.pos
        self.legend_bg.size = self.legend_panel.size
    
    def _add_legend_item(self, color, label):
        """Додає елемент до легенди"""
        item = BoxLayout(orientation='horizontal', size_hint=(None, 1), spacing=5)
        item.width = 100
        
        # Кольоровий квадратик
        color_box = Label(size_hint=(None, 1), width=20, text='■', 
                          markup=True, font_size=20)
        if isinstance(color, (list, tuple)) and len(color) >= 3:
            hex_color = '%02x%02x%02x' % (int(color[0]*255), int(color[1]*255), int(color[2]*255))
            color_box.text = f'[color={hex_color}]■[/color]'
        
        # Текст
        text_label = Label(text=label, size_hint=(1, 1), font_size=12, 
                          color=rgb('00ffb3'), halign='left', valign='middle')
        text_label.bind(size=text_label.setter('text_size'))
        
        item.add_widget(color_box)
        item.add_widget(text_label)
        
        self.legend_panel.add_widget(item)
    ...
    def add_plot(self, color=None, label=""):
        """Додає новий плот до графіка
        
        Args:
            color: колір лінії у форматі [r, g, b, a] або hex string
            label: назва для легенди
            
        Returns:
            індекс доданого плоту
        """
        if color is None:
            color = rgb('00Af00')
        ...
        plot.points = []
        plot.label = label  # зберігаємо назву для легенди
        
        self.graph.add_plot(plot)
        self.plots_list.append(plot)
        if label:
            self._add_legend_item(color, label)
        
        return len(self.plots_list) - 1
    
    def add_yPlot(self, points_list, color, label=""):
        """Додає плот з посиланням на існуючий список точок
        Args:
            points_list: посилання на список, який буде оновлюватися
            color: колір лінії
            label: назва для легенди
        """
        ...
        plot = SmoothLinePlot(color=color)
        plot.points = [(i, y) for i, y in enumerate(points_list)]  # Конвертуємо в (x,y)
        plot.label = label
        plot._source_list = points_list  # Зберігаємо посилання
        self.graph.add_plot(plot)
        self.plots_list.append(plot)
        if label:
            self._add_legend_item(color, label)
    ...
    def refresh_plots(self):
        """Оновлює всі плоти з їхніх списків-джерел"""
        for plot in self.plots_list:
            if hasattr(plot, '_source_list'):
                plot.points = [(i, y) for i, y in enumerate(plot._source_list)]

    def add_y_point(self, y):
        # Додаємо нову точку
        self.plot.points.append((self.current_x, y))

        # Якщо точок стало більше, видаляємо першу
        if len(self.plot.points) > 1000:
            self.plot.points.pop(0)

        # Зсуваємо вікно по X, щоб графік "їхав"
        self.current_x += 1
        self.graph.xmin = self.current_x - (self.xmax - self.xmin)
        self.graph.xmax = self.current_x
...
if __name__ == '__main__':
    ...
    class TestApp(App):
        ...
        def build(self):
            ...
            self.plot = SmoothLinePlot(color=rgb('00Af00')) # next(colors)
            self.plot.points = [(x / 10., sin(x / 50.)) for x in range(-500, 501)]
