...
class SmartHomeGatewayUdpClient:
    ...
    def sendCanMessage(self, deviceId=0, msgType=1, idType=1, can_id=0, data_bytes=[0]*8):
        ...
        * Byte [20]      : uint8_t   endMarker     (Must be 0x55)
        """
        #print(f"Preparing to send CAN message: deviceId={deviceId}, msgType={msgType}, idType={idType}, can_id=0x{can_id:08X}, data_bytes={data_bytes}")
        try:
            ...
            packet.append(0x55)
            # print packet as hex - AA 11 22 BB 00 02 01 00 01 00 00 08 00 01 64 00 3E 26 00 00 55
            #print(f"Constructed CAN packet: {packet.hex().upper()}")
            # Відправляємо пакет
            if self.gatewayIp:
