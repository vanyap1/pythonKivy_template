...
_image_dir = os.path.join(os.path.dirname(__file__), 'assets')
...
example using fractions of PI."""
    ...
    def _draw_values(self):
        ...
        # Create labels with proper kivy.core.text.Label usage
        values = []
        for i in value_list:
            label_text = value_str(i)
            label = CoreLabel(text=label_text, bold=True, font_size=self.value_font_size)
            label.refresh()  # Call refresh() to generate texture
            values.append(label)
    ...
    def _draw_label(self, *t):
        ...
        if self.label_icon:
            ...
            label = CoreLabel(text=self.label, markup=True, bold=True,
...
if __name__ == '__main__':
    ...
    class SpeedMeterTestApp(App):
        def build(self):
            ...
            # Додаємо кольорові сектори
            #self.speedmeter.sectors = [
            #    0, '#00ff00',    # Зелений 0-60
            #    60, '#ffff00',   # Жовтий 60-120
            #    120, '#ff0000'   # Червоний 120-180
            #]
            
            main_layout.add_widget(self.speedmeter)
            
            # Контрольна панель
            control_layout = BoxLayout(orientation='vertical', size_hint=(1, 0.3), spacing=5)
            
            # Слайдер для ручного контролю
            slider_layout = BoxLayout(orientation='horizontal', size_hint_y=None, height=50)
            slider_layout.add_widget(Label(text='Value:', size_hint_x=0.2))
            ...
            self.value_label = Label(text='0', size_hint_x=0.2)
            slider_layout.add_widget(self.value_label)
            
            control_layout.add_widget(slider_layout)
            
            # Кнопки для демонстрації
            button_layout = BoxLayout(orientation='horizontal', size_hint_y=None, height=50, spacing=5)
            
            btn_random = Button(text='Random Value')
            btn_random.bind(on_press=self.set_random_value)
            button_layout.add_widget(btn_random)
            
            btn_animate = Button(text='Animate')
            btn_animate.bind(on_press=self.start_animation)
            button_layout.add_widget(btn_animate)
            
            btn_reset = Button(text='Reset')
            btn_reset.bind(on_press=self.reset_value)
            ...
            # Запускаємо анімацію при старті
            Clock.schedule_once(lambda dt: self.start_animation(None), 1)
            
            return main_layout
        
        def on_slider_value(self, instance, value):
            """Оновлення значення через слайдер"""
            self.speedmeter.value = value
            self.value_label.text = str(int(value))
        
        def set_random_value(self, instance):
            """Встановлення випадкового значення"""
            value = random.randint(0, 180)
            self.speedmeter.value = value
            self.slider.value = value
            self.value_label.text = str(value)
        
        def reset_value(self, instance):
            """Скидання значення до 0"""
            self.speedmeter.value = 0
            self.slider.value = 0
            self.value_label.text = '0'
        
        def start_animation(self, instance):
            """Запуск плавної анімації"""
            self.animation_target = random.randint(20, 160)
            self.animation_step = 2 if self.animation_target > self.speedmeter.value else -2
            Clock.schedule_interval(self.animate_step, 0.05)
        
        def animate_step(self, dt):
            """Крок анімації"""
            current = self.speedmeter.value
            target = self.animation_target
            
            if abs(current - target) <= abs(self.animation_step):
                self.speedmeter.value = target
                self.slider.value = target
                self.value_label.text = str(int(target))
                return False  # Зупинити анімацію
            
            new_value = current + self.animation_step
            self.speedmeter.value = new_value
            self.slider.value = new_value
            self.value_label.text = str(int(new_value))
            
            return True  # Продовжити анімацію

    SpeedMeterTestApp().run()
